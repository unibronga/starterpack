...
const optimization = () => {
    const config = {
        splitChunks: { // схлопывает одинакоые файлы кода в файлы с префиксом vendor
            chunks: 'all'
        }
    }
    if (isProd) {
...
module.exports = {
    ...
    devServer: { // webpack-dev-server (запускает сервер и позволяет отслеживать изменения в реальном времени)
        port: 4200,
        open: true,
        hot: isProd
    },
    ...
    resolve: { // разрешение
        ...
        alias: { // резервируем сокращения для путей
    ...
    // Лоадеры:
    module: {
        rules: [  // правила для неизвестных типов файлов
            {
                test: /\.s[ac]ss$/i,
                use: [  // Указываем лоадер который загрузили (правило действует с право на лево <---)
                    isDev ?
